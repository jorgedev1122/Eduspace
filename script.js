...
document.getElementById("card-tarefas")?.addEventListener("click", () => {
  document.querySelector('.sidebar li[data-target="tarefas"]')?.click();
});
...
function expandirMensagem(elem) {
  if (elem.classList.contains("expanded")) {
    ...
    document
      .querySelectorAll(".mensagem-item")
      .forEach((m) => m.classList.remove("expanded"));
...
const tarefasData = {
  1: {
    ...
    dificuldade: "Difícil",
    enunciado:
      "Resolva as seguintes equações do 1º grau e mostre o passo a passo:",
  ...
  2: {
    ...
    dificuldade: "Médio",
    enunciado:
      "Escreva uma redação dissertativa sobre o tema: 'A importância da tecnologia na educação'",
  ...
  5: {
    ...
    dificuldade: "Médio",
    enunciado:
      "Practice a conversation about daily routines with proper English grammar.",
...
taskCardsNew.forEach((card) => {
  card.addEventListener("click", () => {
    ...
    if (taskData) {
      ...
      document.getElementById("detalhe-professor").textContent =
        taskData.professor;
      ...
      document.getElementById("detalhe-dificuldade").textContent =
        taskData.dificuldade;
      document.getElementById("detalhe-enunciado-texto").textContent =
        taskData.enunciado;
